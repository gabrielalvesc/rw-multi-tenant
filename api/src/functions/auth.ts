...
export const handler = async (
  ...
  const authHandler = new AuthHandlerSogo(event, context, {
...
class AuthHandlerSogo extends DbAuthHandler<unknown> {
  ...
  async login() {
    const bodyContent = JSON.parse(this.event.body.toString())
    ...
    if (type === LOGIN_TYPE.ADMIN) {
      if (!user.roles.some((role) => role.id === 1)) {
        throw {
          exception: 'system.messages.you-cannot-access',
        }
      }
    } else if (type === LOGIN_TYPE.PUBLIC_AGENT) {
      if (user.publicAgents.length === 0) {
        throw {
          exception: 'system.messages.no-public-agents',
        }
      }

      if (!user.roles.some((role) => role.id === 2)) {
        throw {
          exception: 'system.messages.you-cannot-access-this-enviroment',
        }
      }

      if (
        !user.publicAgents.some(
          (publicAgent) => publicAgent.instanceId === tenantId
        )
      ) {
        throw {
          exception: 'system.messages.you-cannot-access-this-instance',
        }
      }
    } else {
      if (!user.roles.some((role) => role.id === 3)) {
        throw {
          exception: 'system.messages.you-cannot-access-this-enviroment',
        }
      }
    ...
    if (hashedPassword !== user.hashedPassword) {
      throw {
        exception: 'system.messages.incorrect-password',
      }
    }
